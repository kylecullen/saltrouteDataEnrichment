...
def fetch_first_20_people(conn):
    """
    Retrieves the first 20 people's ID, full name, and surname
    from the 'dbo.MasterItems' table.
    """
    if not conn:
    ...
    people_data = []
    try:
        # SQL query to select Id, FullName, and Surname from the 'dbo.MasterItems' table
        # Removed 'Gender' and 'Language' as they caused 'Invalid column name' errors.
        query = "SELECT TOP 20 Id, FullName, Surname FROM dbo.MasterItems ORDER BY Id ASC"
        ...
        if rows:
            ...
            for row in rows:
                # Access columns by index (0 for Id, 1 for FullName, 2 for Surname)
                person_id, full_name, surname = row # Removed gender and language from unpacking
                people_data.append({
                    'id': person_id,
                    'full_name': full_name,
                    'surname': surname
                })
                print(f"ID: {person_id}, Full Name: {full_name}, Surname: {surname}") # Removed gender and language from print
        else:
